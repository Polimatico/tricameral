...
<head>
    ...
    <title>Issues · {{ $project->name }}</title>
    ...
    <main class="flex-1 px-6 py-10 max-w-4xl mx-auto w-full">
        ...
        <div class="mb-6 border border-white/10 rounded-xl overflow-hidden">
            <div class="px-4 py-2.5 border-b border-white/8 bg-white/2">
                <span class="text-white/40 text-xs font-medium uppercase tracking-wider">{{ __('messages.open_new_issue') }}</span>
            </div>
            <div class="p-4">
                <form method="POST" action="{{ route('projects.issues.store', $project) }}">
                    ...
                    <input
                        type="text"
                        name="title"
                        placeholder="{{ __('messages.issue_title_ph') }}"
                        value="{{ old('title') }}"
                    ...
                    @include('user.partials.md_editor', [
                        ...
                        'value' => old('body', ''),
                        'rows' => 4,
                        'placeholder' => __('messages.issue_body_ph'),
                        'size' => 'sm',
                    ])
                    @error('body')
        ...
        @if ($project->isAdminFor(Auth::user()))
            <div class="mb-6 border border-white/10 rounded-xl overflow-hidden">
                ...
                <div class="p-4">
                        ...
                        <div class="flex flex-wrap gap-2 mb-4">
                            @foreach ($projectTags as $tag)
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium"
                                    ...
                                    {{ $tag->label }}
                                    <form method="POST" action="{{ route('projects.issue_tags.destroy', [$project, $tag]) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @endif
                    <form method="POST" action="{{ route('projects.issue_tags.store', $project) }}" class="flex items-end gap-3">
                        @csrf
                        <div class="flex-1">
                            <input
                                type="text"
        ...
        <div class="border border-white/10 rounded-xl overflow-hidden">
            <div class="px-4 py-2.5 border-b border-white/8 bg-white/2 flex items-center justify-between">
                <span class="text-white/40 text-xs font-medium uppercase tracking-wider">{{ __('messages.issues_open_header') }}</span>
                <span class="text-white/20 text-xs">{{ $issues->count() }} {{ $issues->count() === 1 ? __('messages.issue_singular') : __('messages.issue_plural') }}</span>
            </div>

            @if ($issues->isEmpty())
                <div class="px-5 py-12 text-center text-white/25 text-sm">
                    {{ __('messages.no_issues_open') }}
                </div>
            @else
                        ...
                        <a href="{{ route('projects.issues.show', [$project, $issue]) }}"
                           ...
                           class="flex items-center gap-4 px-5 py-4 hover:bg-white/3 transition-colors group no-underline">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                ...
                                @else
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0" style="color: rgb(255 255 255 / 0.25)" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs mt-0.5 truncate" style="color: rgb(255 255 255 / 0.25)">
                                        #{{ $issue->id }} · {{ $issue->status === 'open' ? __('messages.status_open') : __('messages.status_closed') }} {{ __('messages.from') }} {{ $issue->user->name }} · {{ $issue->created_at->diffForHumans() }}
                                    </p>
                                </div>
                            </div>
