...
<head>
    ...
    <title>{{ __('messages.tab_merge') }} · {{ $project->name }}</title>
    ...
    <main class="flex-1 px-6 py-10 max-w-4xl mx-auto w-full">
        ...
        <div class="border border-white/10 rounded-xl overflow-hidden">
            <div class="px-4 py-2.5 border-b border-white/8 bg-white/2 flex items-center justify-between">
                <span class="text-white/40 text-xs font-medium uppercase tracking-wider">{{ __('messages.merge_history') }}</span>
                <span class="text-white/20 text-xs">{{ $merges->count() }} {{ $merges->count() === 1 ? __('messages.merge_singular') : __('messages.merge_plural') }}</span>
            </div>

            @if ($merges->isEmpty())
                <div class="px-5 py-12 text-center text-white/25 text-sm">
                    {{ __('messages.no_merges_yet') }}
                </div>
            @else
                <div class="divide-y divide-white/6">
                    @foreach ($merges as $merge)
                        <a href="{{ route('projects.merge.show', [$project, $merge]) }}"
                           class="flex items-start gap-4 px-5 py-4 hover:bg-white/3 transition-colors group no-underline">

                            <div class="mt-0.5 shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: rgb(129 140 248 / 0.7)">
                                    <circle cx="18" cy="18" r="3"/><circle cx="6" cy="6" r="3"/><path d="M6 21V9a9 9 0 0 0 9 9"/>
                                </svg>
                            </div>

                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-white/75 truncate group-hover:text-white/90 transition-colors">
                                    {{ $merge->title }}
                                </p>
                                <p class="text-xs mt-0.5 text-white/25">
                                    #{{ $merge->id }} · {{ __('messages.merged_by_label') }} <span class="text-white/40">{{ $merge->user->name }}</span> · {{ $merge->updated_at->diffForHumans() }}
                                    @if ($merge->sourceProject)
                                        · {{ __('messages.from') }} <span class="text-white/35">{{ $merge->sourceProject->name }}</span>
                                    @endif
                                </p>
                            </div>

                            <div class="flex items-center gap-3 shrink-0 mt-0.5">
                                @if ($merge->tags->isNotEmpty())
                                    <div class="flex items-center gap-1.5">
                                        @foreach ($merge->tags as $tag)
                                ...
                                @endif
                                @if ($merge->comments_count > 0)
                                    <div class="flex items-center gap-1 text-xs text-white/25">
                                        ...
                                        <span>{{ $merge->comments_count }}</span>
                                    </div>
                                @endif
                                <span class="font-mono text-xs text-white/20">#{{ $merge->id }}</span>
                            </div>
                        </a>
                    @endforeach
                </div>
